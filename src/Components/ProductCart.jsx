    ...
    <>
      <MAINDIV>
          ...
          <METADATA>
            ...
            <div className="rating">
              <Flex color={'var(--primary2)'}>
                {fs.current?.map((el, index) => (
                  <FontAwesomeIcon fontSize={'0.8rem'} key={el + index} icon={faStar} />
                ))}
                {hs.current?.map((el, index) => (
                  <FontAwesomeIcon fontSize={'0.8rem'} key={el + index} icon={faStarHalf} />
                ))}
                {es.current?.map((el, index) => (
                  <FontAwesomeIcon fontSize={'0.8rem'} key={el + index} icon={['far', 'star']} />
                ))}
              </Flex>
              <p>({numVotes})</p>
            </div>
        ...
        <Button
          ...
          leftIcon={<HiOutlineShoppingBag style={{ fontSize: '1.3rem' }} />}
        >
          ADD TO CART
        </Button>
